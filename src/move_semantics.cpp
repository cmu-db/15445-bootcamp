...
// that refer to an location in memory. Rvalues are anything that is not a lvalue.

// std::move is the most common way of moving an object from one lvalue to another.
// std::move casts an expression to an rvalue. This allows for us to interact with
// an lvalue as an rvalue, and allows for the ownership to be transferred from one
// lvalue to another.

// In the code below, we include some examples for identifying whether expressions
// in C++ are lvalues or rvalues, how to use std::move, and passing rvalues 
...
// and the type std::vector<int> is an array of ints. Mainly, vectors take up
// a non-negligible amount of memory, and are here to show the performance benefits
// of using std::move.
#include <vector>
...
int main() {
  ...
  // Rvalue references are references that refer to the data itself, as opposed to an
  // lvalue. Calling std::move on a lvalue (such as stealing_ints) will result in the 
  // expression being casted to an rvalue reference.
  ...
  // stealing_ints, since that is the lvalue that owns the data, not rvalue_stealing_ints.
  std::cout << "Printing from stealing_ints: " << stealing_ints[1] << std::endl;

  // It is possible to pass in an rvalue reference into a function. However, once the rvalue
  // is moved from the lvalue in the caller context to an lvalue in the callee context, it is
  // effectively unusable to the caller. Essentially, after move_add_three_and_print is called,
  // we cannot use the data in int_array2. It no longer belongs to the int_array2 lvalue.
  std::vector<int> int_array2 = {1, 2, 3, 4};
  std::cout << "Calling move_add_three_and_print...\n";
  move_add_three_and_print(std::move(int_array2));

  // It would be unwise to try to do anything with int_array2 here. Uncomment the code to
  // try it out! (On my machine, this segfaults...)
  // NOTE: THIS MIGHT WORK FOR YOU. THIS DOES NOT MEAN THAT THIS IS WISE TO DO!
  // std::cout << int_array2[1] << std::endl;

  // If we don't move the lvalue in the caller context to any lvalue in the callee context,
  // then effectively the function treats the rvalue reference passed in as a reference, and
  // the lvalue in this context still owns the vector data.
