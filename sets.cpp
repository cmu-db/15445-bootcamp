...
// resource as you complete the assignments in this class, so you should check it out!
...
int main() {
  // The std::set container is a data structure that contains a sorted set of unique objects of
  // a single type. It is usually implemented as a Red-Black tree, if that helps you conceptualize
  // the std::set. The std::set container is used to maintain of a set of unique elements.

  // We can declare a int set with the following syntax.
  std::set<int> int_set;

  // To insert elements, we use the insert function. Here we insert elements 1 through 10
  // in our set. There also exists an emplace function that allows the user to construct
  // objects in place for set insertion. We cover emplace more in vectors.cpp (line 73).
  for (int i = 1; i <= 10; ++i) {
    int_set.insert(i);
  }

  // To find an element, we can use the find function, which returns an iterator to the 
  // element within the set with the key that is equivalent to the key argument. We can 
  // then check to see whether this returned iterator is equivalent to the end iterator, 
  // which would imply that the element does not exist.
  std::set<int>::iterator search = int_set.find(2);
  if (search != int_set.end()) {
    std::cout << "Element 2 is in int_set.\n";
  }

  // We can also use the count function, which returns the number of elements with the 
  // specified key in the set.
  ...
  // To erase an element, we can use the erase function. The erase function can firstly,
  // take a key to erase. For instance, if we want to erase 4 from the set, we can call:
  int_set.erase(4);
  ...
  // Lastly, we can erase elements in the set by passing in an iterator range to the 
  // erase function. For instance, if we want to erase elements that are greater than
  // or equal to 9 (so, 9 and 10), we call the following.
  ...
  // We can iterate through the set elements via the set iterator. You cannot iterate
  // through a set via indexes of any kind.
  ...
  // Just like std::vector, we can also iterate through the set via a for-each loop.
